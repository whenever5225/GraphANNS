...
@File: eva_modal1_brute_force_node.h
@Time: 2022/11/1 8:36 PM
@Desc: modal1 brute force
***************************/

#ifndef GRAPHANNS_EVA_MODAL1_BRUTE_FORCE_NODE_H
#define GRAPHANNS_EVA_MODAL1_BRUTE_FORCE_NODE_H
...
public:
    ...
    CStatus run() override {
        ...
        s_param->results_modal1.resize(num_);
...
#pragma omp parallel for num_threads(Params.thread_num_) schedule(dynamic) \
        ...
        for (IDType i = 0; i < query_num_; i++) {
            ...
            for (IDType j = 0; j < top_k; j++) {
                ...
                dist_id.pop();
                s_param->results_modal1[i].emplace_back(p.second);
            }
        }
...
#endif //GRAPHANNS_EVA_MODAL1_BRUTE_FORCE_NODE_H
